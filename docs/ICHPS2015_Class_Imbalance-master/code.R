...
table(testing$Class)/nrow(testing)
...
ggplot(rf_mod$results, aes(x = mtry, y = ROC)) + geom_point() + geom_line() + 
  ...
  geom_line(data = rf_down_int$results, aes(x = mtry, y = ROC), col = rf_mod$mod_cols[2]) + 
  theme_bw() + 
...
c5_costs_res <- subset(c5_costs$results, trials <= 5)
